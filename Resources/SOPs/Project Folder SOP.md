...
**Audience:** Sam (routing), all working personas.
...
## Sam's role at routing

When Sam receives a request, he assesses whether it is project-eligible **before** routing to a team member.
...
Sam does not create a project folder speculatively or mid-task. If a team member discovers mid-work that a project folder is needed, they surface it to Sam — Sam then proposes and seeks approval before proceeding.
...
Sam creates the minimum structure needed. A typical project folder:
...
Not every project needs all three. Sam creates only what the task requires.

---

## Template subfolder

`Projects/Template/` is reserved as a blank scaffold for reference. Sam never uses it as a working project folder — always create a new named folder.

---

## Handoff

Once the project folder exists and the task is complete, the working persona notifies Sam. Sam confirms the deliverables are in `Deliverables/` and the folder is tidy before closing the task.
