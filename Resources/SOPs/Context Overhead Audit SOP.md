...
- **Garden loop trigger:** the same persona/sub-agent mistake observed twice → queue a candidate rule via step 7 (Log), at the lowest-cost tier that catches it (folder-tier `AGENTS.md` or an SOP, before root `AGENTS.md`).
...
5. **Check folder-tier pointer validity.** For each folder-tier `AGENTS.md` (see [Folder-Tier CLAUDE.md SOP](Folder-Tier%20CLAUDE.md%20SOP.md) for the canonical list), confirm its cited SOP still exists and still carries the rules the folder file points to — a broken or stale pointer is drift.
6. **Garden the rulebook.** Two candidate sources for pruning, both routed through step 7 (Log) — no separate channel: (a) any rule now observed as default model behaviour → candidate for deletion; (b) for each root `AGENTS.md` rule, could it demote to a folder-tier file or SOP instead? This step only *nominates* — additions/deletions/demotions are governance-artefact edits taking the full pipeline (Checkpoints A/B; root `AGENTS.md` stays Orchestrator-only).
...
- Not a licence to edit `AGENTS.md` or `CLAUDE.md` files or SOPs inline mid-audit — the audit only queues candidates.
