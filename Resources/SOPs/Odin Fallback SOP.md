...
> **Not this fallback:** a deterministic `cannot be used as an advisor when the request model is '<model>'` error is a *config* failure, not a transient outage — retrying will fail identically. It means the `advisorModel` setting (user-level) is weaker than the gatekeeper's request model. Odin runs on `claude-fable-5` and Quinn on `claude-opus-4-8` (gatekeeper tier). Because `advisorModel` is user-level (not shipped in the repo), each clone owner must set it ≥ the highest gatekeeper pin — here `claude-fable-5` (or the `fable` alias). A fresh clone that leaves `advisorModel` weaker will hit this error on Odin dispatches until the owner raises it (see Advisor Checkpoints SOP § pairing). Do not substitute self-review for it — correct the setting and re-dispatch.
