...
|---|---|---|---|
| Gatekeeper | `claude-fable-5` | high | Gatekeeping authority G=5: holds genuine PASS/BLOCKED verdict power over others' deliverables, with direct error-cascade consequences. Currently: Senior Adviser, QA Compliance Reviewer (on `claude-fable-5`, the top tier — post-promo access confirmed 25/07/2026). Fable 5 safety-classifier refusals are surfaced to the Orchestrator and re-run on `claude-opus-5` at dispatch — a dispatch override on the gatekeeper, never a change to the gatekeeper's frontmatter pin. |
...
| Production | `claude-sonnet-5` | high | All other personas. Default for new hires unless Gatekeeper criteria are met at hire time. |
...
Effort is set via each persona's frontmatter `effort:` key (overrides the session dial when set, inherits it when omitted); the column above is the durable convention, and the frontmatter key is the carrier. Effort is uniformly `high` across all three tiers per the per-role review of 11/08/2026: the Gatekeeper tier was raised from `medium` on verification-thoroughness evidence — the dimension effort controls — while the ≤100-word checkpoint output cap keeps the cost bounded. The column stays per-tier and may diverge again on evidence; the 07/08/2026 inverted-gradient rationale is retired.
