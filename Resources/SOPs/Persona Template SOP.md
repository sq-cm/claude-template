...
Effort is set via each persona's frontmatter `effort:` key (overrides the session dial when set, inherits it when omitted); the column above is the durable convention, and the frontmatter key is the carrier. The effort gradient runs deliberately opposite the model gradient: the more capable the pinned model, the lower the effort it needs, since a stronger model reaches the same answer with less deliberation and the token saving lands where it costs most (decided 07/08/2026).
