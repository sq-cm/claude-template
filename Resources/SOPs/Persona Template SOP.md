...
|---|---|---|---|
| Gatekeeper | `claude-fable-5` / `claude-opus-5` | high | Gatekeeping authority G=5: holds genuine PASS/BLOCKED verdict power over others' deliverables, with direct error-cascade consequences. Currently: Senior Adviser (`claude-fable-5`, the top tier — sole Fable pin; post-promo access confirmed 25/07/2026) and QA Compliance Reviewer (`claude-opus-5` — moved from Fable 5 on cost/efficiency 13/08/2026, verdict authority unchanged). Fable 5 safety-classifier refusals (Senior Adviser only) are surfaced to the Orchestrator and re-run on `claude-opus-5` at dispatch — a dispatch override on the gatekeeper, never a change to the gatekeeper's frontmatter pin. |
| Judgement | `claude-opus-5` | high | Judgement-heavy production personas elevated above the Production default without gatekeeping authority. Currently: HR Lead, Senior Researcher, Copywriter, Business Analyst, Brand Strategist, Legal and Compliance Writer. Near-Fable capability at half the price (Opus 5, released 24/07/2026); revert target `claude-sonnet-5`. A flagged Opus 5 request auto-falls-back to `claude-opus-4-8` in Claude Code — a platform-level fallback, outside roster control — surfaced to the Orchestrator as a blocker, never a silent drop. |
| Production | `claude-sonnet-5` | xhigh | All other personas. Default for new hires unless Gatekeeper criteria are met at hire time. |
...
Effort is set via each persona's frontmatter `effort:` key (overrides the session dial when set, inherits it when omitted; harness enum: `low`/`medium`/`high`/`xhigh`/`max`, available levels depend on the model); the column above is the durable convention, and the frontmatter key is the carrier. Gatekeeper and Judgement tiers run `high` (the Gatekeeper tier was raised from `medium` on verification-thoroughness evidence per the per-role review of 11/08/2026, with the ≤100-word checkpoint output cap keeping cost bounded); the Production tier runs `xhigh` per maintainer decision 13/08/2026, superseding that review's uniform-`high` setting for this tier only. The column stays per-tier and may diverge again on evidence.

A top-tier promotion may elevate a decided set of judgement-heavy production personas above the `claude-sonnet-5` Production default. Currently elevated to `claude-opus-5` (Judgement tier): HR Lead, Senior Researcher, Copywriter, Business Analyst, Brand Strategist, Legal and Compliance Writer. The durable signal for any such promotion is each promoted persona's frontmatter `model:` pin together with the tier table above; the persona's in-file model note is a readable pointer to this SOP, not the system of record. The revert target for the Judgement tier is `claude-sonnet-5`. `claude-opus-4-8` is retired from the roster (25/07/2026). The dispatch-time fallback for gatekeeper Fable 5 refusals is `claude-opus-5` (released 24/07/2026, superseding Opus 4.8 at the same price) — in that fallback role it is a dispatch override on the gatekeeper, not a change to the gatekeeper's frontmatter pin.
