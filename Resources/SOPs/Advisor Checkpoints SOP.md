...
**Advisor-model pairing (load-bearing).** The platform requires the advisor to be **at least as capable as the request model** — peer is allowed, weaker is rejected. The gatekeepers are pinned per role: @{SeniorAdviser} to the top tier, `claude-fable-5` (Fable 5 roster migration, 25/07/2026), and @{QAComplianceReviewer} to `claude-opus-5` (moved from Fable 5 on cost/efficiency, 13/08/2026). Each clone owner must set `advisorModel` (a user-level setting the repo cannot ship) ≥ the highest gatekeeper pin — with @{SeniorAdviser} on Fable 5, that means raising `advisorModel` to `claude-fable-5` — otherwise gatekeeper dispatches fail with `cannot be used as an advisor when the request model is '<model>'`. Any clone that raises a gatekeeper above its `advisorModel` must raise `advisorModel` to match.
