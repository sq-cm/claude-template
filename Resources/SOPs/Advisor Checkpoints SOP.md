...
**Advisor-model pairing (load-bearing).** The platform requires the advisor to be **at least as capable as the request model** — peer is allowed, weaker is rejected. Both gatekeepers — @{SeniorAdviser} and @{QAComplianceReviewer} — are pinned to the top available tier, `claude-opus-4-8`. Each clone owner must set `advisorModel` (a user-level setting the repo cannot ship) ≥ the gatekeeper pin — otherwise gatekeeper dispatches fail with `cannot be used as an advisor when the request model is '<model>'`. Any clone that raises a gatekeeper above its `advisorModel` (for example, a future re-promotion to a flagship tier above Opus) must raise `advisorModel` to match.
