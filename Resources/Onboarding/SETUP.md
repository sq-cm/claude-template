...
**herdr** ([herdr.dev](https://herdr.dev)) — auto-installed at `.claude/commands/onboard.md` Step 14 via the vendor's official pipe-to-shell installer (Windows: `irm https://herdr.dev/install.ps1 | iex`; macOS/Linux: `curl -fsSL https://herdr.dev/install.sh | sh`). Windows installs the preview channel by default — the only channel available on that platform. `Vault/Scripts/tool-check.sh` checks it against GitHub releases the same way it checks plannotator; herdr updates itself via `herdr update` (never run automatically — see the tool's own docs for why).
