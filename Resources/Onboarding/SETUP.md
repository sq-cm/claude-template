...
**plannotator** ([github.com/backnotprop/plannotator](https://github.com/backnotprop/plannotator)) adds visual review of agent plans and code diffs — approve/deny plans with inline annotations, review git diffs and PRs, send feedback back to the agent. Its Claude Code plugin registers automatically like the rest of the roster; the plannotator *binary* is a separate, auto-run install (`.claude/commands/onboard.md` Step 10, no consent gate) — a checksum-verified download of the **latest** release (unpinned; see accepted-risk note below), not a pipe-to-shell installer. The binary then keeps itself current: the SessionStart hook refreshes it once a day through `Vault/Scripts/tool-check.sh --apply` — the same script and the same checksum-verified path Step 10 uses — and `/update` applies it too. On Windows that is the whole story. On macOS and Linux the last step needs root. The script uses `sudo -n`, which never prompts, so without a cached sudo credential — the normal state — each new release ends with the file downloaded and verified and a one-line `sudo install` command printed for you to run.
