...
Paste this to check all three at once:

```bash
for c in bash jq git; do command -v "$c" >/dev/null 2>&1 && echo "ok   $c" || echo "MISS $c"; done
```
...
- **Not editable for commit** — everything else (AGENTS.md, CLAUDE.md, `.claude/agents/`, SOPs, settings, etc.)
...
| File | Purpose |
|------|---------|
| `AGENTS.md` | System bible — how Sam and the team work |
...
**herdr** ([herdr.dev](https://herdr.dev)) — optional, and **not** installed or version-checked by this vault. Onboarding used to install it automatically; that step was removed, so install it yourself from [herdr.dev/docs/install](https://herdr.dev/docs/install/) if you want it. Day-to-day usage, the concept model, and troubleshooting live in the [Herdr SOP](../SOPs/Herdr%20SOP.md).

Restart Claude Code after any plugin install.

### Accepted risk — unpinned third-party code paths

Every marketplace in `.claude/settings.json` `extraKnownMarketplaces` sourced from GitHub (`context-mode`, `obsidian-skills`, `caveman`, `higgsfield`, `anthropic-agent-skills`, `thedotmack`) tracks its repo's default branch at HEAD, not a pinned commit or tag. None of the six publishes tags, and commit-SHA refs were tested and rejected by the clone mechanism (`git clone --branch <sha>` fails), so branch-at-HEAD is the only option. `autoUpdate` is explicitly `false` on the caveman entry (previously `true`); the other five entries omit the key, which defaults to `false`. Either way, a fresh trust-prompt install always uses whatever is at HEAD at trust time, and updates only happen when a maintainer explicitly runs `/plugin marketplace update`.

**Privileged installs during `/onboard` — removed rather than accepted.** Earlier versions of `.claude/commands/onboard.md` Step 7 auto-installed Node.js on the user's behalf: on macOS by piping `https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh` into `/bin/bash` at a moving HEAD, and on Linux with `sudo apt-get install -y nodejs npm`. Neither was reviewable before it ran and neither was recorded here. Both are now gone: Step 7 tells the user how to install Node.js and skips Caveman if they have not, because Caveman is an optional token-saving plugin and installing a system-wide package manager to enable it is out of proportion. Windows still uses `winget`, which needs no privilege escalation and pipes nothing into a shell. The onboarding path no longer escalates privilege anywhere. The last exception was a `sudo -n` move that placed a downloaded binary in `/usr/local/bin`, and it went when that binary's onboarding step was removed (see `CHANGELOG.md`). The GitHub CLI (`gh`) is no longer used anywhere in onboarding either.

The daily template pull adds no privilege escalation — it is plain git against the template repository. `update.sh --unattended`, run by the SessionStart hook once a day at most, only fires when the vault is already on `local/main` with a clean working tree — plus local `main` not diverged from `origin/main`, no rebase or merge already in progress, network reachable, no other session holding the update lock, and not a maintainer clone (`CLAUDE_TEMPLATE_MAINTAINER=1` skips it outright). When those hold, it fast-forwards `main` to `origin/main` and rebases `local/main` on top. A conflicting rebase is cancelled automatically and `main` is reset to its pre-run SHA, with the conflicting files listed in the session context — unlike interactive `/update`, which deliberately leaves the repo mid-rebase for you to resolve. Run `/update` when you're ready to resolve it interactively. If that automatic recovery itself fails, a `RECOVERY FAILED` line is reported in the session context instead.
...
**Retired marketplace — `coreyhaines31/marketingskills`.** Removed from `extraKnownMarketplaces` on 04/09/2026 (plan 121, maintainer decision of 03/09/2026). Its plugin, `marketing-skills@marketingskills`, had already been unbundled from `enabledPlugins`, onboarding and `REQUIRED_KEYS` on 04/08/2026 (CHANGELOG entry "(#263)"), and nothing in the roster depended on the registration. It is now a two-command local opt-in rather than one: `/plugin marketplace add coreyhaines31/marketingskills` then `/plugin install marketing-skills@marketingskills`. A clone that already registered the marketplace keeps that registration — it is local Claude Code state (`~/.claude/plugins/known_marketplaces.json`), not something this repository controls — and `/plugin marketplace remove marketingskills` clears it. If you would rather declare it in git-ignored `.claude/settings.local.json` under `extraKnownMarketplaces`: the Claude Code docs state that list-valued keys such as `permissions.allow` merge across settings files, but do not say how an object-valued key like `extraKnownMarketplaces` combines, so after adding it run `/plugin marketplace list` and confirm the six template marketplaces are still registered — if any are missing, copy the template's full `extraKnownMarketplaces` block into the local file alongside your addition (docs checked 04/09/2026).
...
Eight plugins are enabled by default in `.claude/settings.json` `enabledPlugins`. Five are third-party (`context-mode`, `obsidian`, `caveman`, `claude-mem`, `higgsfield`); three are Anthropic's own — `document-skills@anthropic-agent-skills` (from `anthropics/skills`), plus `skill-creator` and `frontend-design` (from `claude-plugins-official`). They install at the trust prompt, before the user has reviewed anything.
...
**To opt out**, set the plugin to `false` in `.claude/settings.json` `enabledPlugins`, and in the same change remove its `tier2_plugin_<name>` entry from `REQUIRED_KEYS` in `.claude/hooks/session-start-onboarding.sh`, as the disabling paragraph above says. For every `tier2_plugin_*` plugin, that is the only roster site to edit: since plan 128 the hook derives its install check, the migration branch's key list and the flag→step map from `REQUIRED_KEYS`. A stale entry makes onboarding re-fire every session, forever, and `Vault/Scripts/validate.sh` Check 14 fails on it. `superpowers` was exactly this case from 17/07/2026 until plan 060 fixed it. Caveman is the one exception: its key is `tier2_caveman`, not a `tier2_plugin_*` key, so it also appears by name in the hook's `SKIPPABLE_KEYS`, install-check loop, Step 7 case line and flag→step map. `higgsfield` is currently enabled, so its `REQUIRED_KEYS` entry is satisfiable and it is a latent case, not a live one.
