# Velora Studio — Hire Paid Media Specialist

## What this is
A hiring pipeline exercise. Velora Studio has a paid media campaign to run — and no one on the current team handles paid advertising. You'll run the full hiring process: Sam's gap note → Ryan's research brief → Harper's persona file → Sam's roster announcement.

## Learning objectives
- How the Orchestrator identifies and escalates a capability gap
- How the Senior Researcher researches a professional role and writes a brief
- How the HR Lead reads the Senior Researcher's brief and builds a persona file from the template
- How the Orchestrator announces a new hire and updates the team roster in `Vault/Memory/theme-name-map.md` (root `AGENTS.md` only points to it)
- **This is the most important meta-workflow** — it teaches how the team grows

## What's already done
- The Orchestrator's gap note (identifies the problem, asks for permission to hire)
- The Senior Researcher's brief: Skills Inventory complete; 3 sections incomplete (Knowledge Domains, Collaboration Patterns, Risk Areas)
- The HR Lead's persona stub: Identity section only; all other sections empty

## What you need to complete

### Step 1 — Complete the Senior Researcher's research brief
1. Open `02 Working/ryan-research-brief.md`
2. Complete the 3 incomplete sections (marked TODO)
3. Save — this is now ready for Harper

### Step 2 — HR Lead drafts the persona
4. Open `02 Working/harper-persona-stub.md`
5. Read the Senior Researcher's completed brief
6. Fill in all remaining persona template sections using Ryan's research
7. Save to `.claude/agents/paid-media-specialist.md`

### Step 3 — Orchestrator announces and updates the roster
8. Open `02 Working/roster-announcement-template.md`
9. Fill in the announcement
10. Update `Vault/Memory/theme-name-map.md` to include Reed (name map entry + file-path table row) — root `AGENTS.md` stays untouched; its `## Theme & Roster` section just points to the name map

## How to start
Tell the Orchestrator: *"I want to run the paid media specialist hiring pipeline."*

The Orchestrator will confirm the gap and route to the Senior Researcher first, then the HR Lead.

## Note
This is the exercise that teaches the system itself. If you understand how hiring works, you understand how to extend the team for any capability gap you encounter in real work.

Once you've run this demo by hand, `/hire <role>` codifies the same sequence for real hires.
