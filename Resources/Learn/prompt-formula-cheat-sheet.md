...
- **Effort is the main cost and speed control.** Set it with `/model` or `--effort`. `medium` is Opus 5.5's default and the right place to start; step up to `high` for harder work and `xhigh` for the hardest coding and agentic work. The vault's own dial runs high or xhigh for plan mode, checkpoints, and architecture, and medium or low once a plan is approved (root `CLAUDE.md` § Default Mode). On your own one-off asks, drop to `low` for routine work — it holds quality there at a fraction of the tokens and latency.
