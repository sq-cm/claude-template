...
Flag→step map: tier1_git_hooks/tier1_env_copy = hook already ran (set true unless FAILED); tier1_node = set true iff Node present; tier2_plugin_claude_mem/context_mode/obsidian/superpowers/impeccable/marketing_skills/document_skills/skill_creator/frontend_design/plannotator = Step 8+9 plugin pairs (plannotator's binary install is a separate optional Step 10 — do not auto-run it; the plugin registration flag here covers only the Claude Code plugin, which auto-installs per settings.json); tier2_caveman = Step 7 + '/caveman lite'; tier2_vscode_git = Step 3."
