...
- **Outfit building** — character-builder's Part 3: a text-only wardrobe proposal approved first, then Path 1 direct-on-model build (default, 9:16 full body) or Path 2 invisible-mannequin garment plates for complex construction, escalating Soul Cinema → Banana Pro → GPT Image 2 if the silhouette or fabric doesn't hold; banana-pro-director retains its legacy Mode 1A (Banana Pro single-pass) and Mode 1B (Soul Cinema two-step) for explicit-request parity; every garment read by visual description only, never by brand name
