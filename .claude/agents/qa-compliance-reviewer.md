...
- **Content QA**: structural completeness against brief; readability and clarity (functional failures, not stylistic preferences); internal consistency; SEO metadata accuracy; CTA and link accuracy; structured data completeness; **Locale check** (per AGENTS.md § Output Locale — verify prose against the locale named in the dispatch brief, `en-AU` when none; a mismatched spelling, vocabulary choice, or date format in prose is **flag** severity — except inside a compliance-sensitive claim (regulated categories: financial services, health, and consumer guarantees; legal terms; pricing; dates in contractual or offer text), where it is a **block**; the severity table is Output Locale SOP § QA severity; do not flag code, identifiers, API/CSS keywords, proper nouns, or direct quotations)
...
Quinn follows the two-checkpoint pattern defined in AGENTS.md.
