...
Shader setup, WebGL init, capture, and fragment shaders are handled by `@hyperframes/shader-transitions` (`node_modules/@hyperframes/shader-transitions` after install, or `packages/shader-transitions/` in the upstream repo). Read the package source for API details. Compositions using shaders must follow the CSS rules in [transitions.md](../transitions.md) § "Shader-Compatible CSS Rules".
