...
These are NOT advanced — they're standard motion design patterns that every composition should use at least 2-3 of. For pre-built UI templates (terminal chrome, device mockups, moodboard layouts), look in the `registry/blocks/` directory of the upstream repo (github.com/heygen-com/hyperframes; not vendored here) instead — those are recipes, not techniques.
...
```bash
python3 .claude/skills/hyperframes/scripts/extract-audio-data.py narration.wav --fps 30 --bands 16 -o audio-data.json
```
...
**Never do:** equalizer bars, spectrum analyzers, waveform displays, strobing, rainbow color cycling. The audio provides _timing and intensity_; the visual vocabulary still comes from the brand. See [`audio-reactive.md`](audio-reactive.md) in this folder for the full API and anti-patterns.
