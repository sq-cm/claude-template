...
(from the ponytail decision-ladder adoption) is local, not upstream. Vault adaptation: plan
...
1. **Never modify source code yourself.** No edits, no fixes, no "quick wins while you're in there." When the audited repo is this vault, the ONLY files you may create or modify live under `Vault/Plans/` (create if absent); when auditing an external repo, they live under `plans/` in that repo's root — or under `advisor-plans/` when `plans/` already exists for an unrelated purpose (create the chosen directory if absent). The `execute` variant dispatches a *separate executor subagent* that edits code in an isolated git worktree — you review its diff and render a verdict; you still never edit code directly, and you never merge, push, or commit to the user's branch.
...
Wait for the selection. Do not write 30 plans nobody asked for. If running non-interactively (no user available to choose), write plans for the top 3–5 by leverage and record that default in `Vault/Plans/README.md`.
...
Before writing anything: record `git rev-parse --short HEAD` — every plan stamps the commit it was written against (the executor uses it for drift detection). If `Vault/Plans/` already exists from a previous run, **reconcile, don't duplicate**: read `Vault/Plans/README.md`, keep numbering monotonic, skip findings already planned or listed as rejected, and mark superseded plans stale in the index. `Vault/Plans/` is shared with the `writing-plans` skill's date-named `YYYY-MM-DD-<name>.md` files — expected neighbours, not an "unrelated purpose"; improve owns `README.md` and `NNN-<slug>.md` only. For an external repo, if `plans/` exists for some unrelated purpose, use `advisor-plans/` instead and say so.
...
Finish by writing `Vault/Plans/README.md` with the recommended execution order, dependencies between plans, and a status column the executor models can update.
