...
>
> **Drop-3 ripple, applied 19/08/2026.** Cinema-director was base-flipped onto upstream drop 3 and its spine moved from thirteen blocks to a **sixteen-slot** order, gated by a Seedance 2.0-vs-2.5 target-version check. This skill adopted no upstream drop-3 content — its twelve-block structure is unchanged — but every inheritance claim was re-reconciled **per block**, not renamed in bulk, because the slots are not a relabelling of the old blocks:
>
> - **Survive under the same name** — Element Tags, Write The Visible, House Rules. Pointers stand as written.
> - **Survive under a new name** — FOV Degree Table → § SLOT 8 — OPTICS (§ FOV degree anchor); Sound Bed → § SLOT 15 — AUDIO; Frame Map → § SLOT 6 — GEOMETRY MAP. Frame Map is a **reversal** of the drop-2 finding: cinema-director had no standalone screen-space block under the thirteen-block spine, and drop 3 reinstated one, so Frame Map moves back out of this skill's legacy bucket and into shared grammar.
> - **Reshaped, not renamed** — Subject Lock was merged with Prop Lock and Corps/Crew Lock into § SLOT 5 — ASSETS, whose line shape (`@tag = identity + THIS SCENE action + fidelity assertion`) differs from the old standalone block; Cross-Frame Rules became § SLOT 16 — LOCKS, restated as a positive ordered chain rather than a list of prohibitions. Both are retained here as this skill's own legacy grammar with the new slot named as the analogue — the "identical in grammar, logic, and rules" claim no longer holds for either.
> - **No longer standalone** — World Plate folded into the SLOT 5 location asset line (`@x_plate = the location — …`); Movement's four-layer structure and cinema-director's former § MOVEMENT AND CHOREOGRAPHY heading are both gone, with the closest analogues at § SLOT 12 — ACTION TIMING (subject motion) and § SLOT 9 — CAMERA (camera motion).
> - **Deleted upstream with no successor** — Last Frame. Cinema-director now opens on § SLOT 7 — FIRST FRAME and carries only an end-state rule inside § SLOT 12 — ACTION TIMING. This skill's Last Frame block is retained legacy grammar; there is no inheritance claim left to make.
> - **Reversed outright** — distributed style. See § CORE PHILOSOPHY below.
...
Five commercial registers replace cinema-director's five cinema modes. The differences across registers are in **light quality, product presentation, movement grammar, grade, and surface treatment** — not in the element-tag grammar, which is shared with cinema-director and operates identically here (cinema-director § ELEMENT TAGS). This skill's twelve-block locked structure is its own independently-maintained format, descended from cinema-director's original ten-block grammar; cinema-director has since moved on twice — to a thirteen-block house format at drop 2, and to a **sixteen-slot spine** at drop 3 — so the two skills have not shared an identical block list for two drops. What is still genuinely shared is the tag-reference mechanics and the core anti-plastic philosophy.

**Density rule.** This skill's own established target — no longer numerically aligned with cinema-director, whose length discipline now targets **900–1,400 words** for a four-shot sequence with four assets: 280–400 words for single-shot scenes, up to 600 for multi-shot. Keep every prompt tight — cut filler. Trust the reference image.

**Distributed style — now this skill's own discipline, no longer shared.** No style header at the top of the prompt: lighting, colour/grade, lens character, skin treatment, product surface treatment, composition, and continuity each live in their home block rather than a top prefix. The prompt opens on Scene & Mood and Frame Map. **This diverged at drop 3.** Cinema-director dropped its § DISTRIBUTED STYLE heading at drop 2 while keeping the discipline, and at drop 3 it reversed the discipline itself — § SLOT 2 — STYLE PREFIX is now a front-loaded invariant capture stack (style, operating style, texture, skin, technical) written once at the top and explicitly "never restated later". Do not carry that pattern across. A front-loaded film-grain-and-photochemical invariant stack contradicts this skill's commercial grade at the first line, and the product-specific aspects it would swallow belong in Product Surface and Brand Grade. Cinema-director's anti-redundancy discipline survives as its § LENGTH DISCIPLINE "every fact lives in exactly one slot" rule, which this skill's per-block placement already satisfies.
...
This skill's own locked format. (Cinema-director's own delivery format has since expanded to three parts — title, tag list, code block, per cinema-director § DELIVERY; this skill keeps its tag list folded into the pre-prompt check rather than repeating it as a separate delivery step.)

**1. Title line with runtime.** Bolded English. Example: `**Seedance commercial prompt — 10s**`

**2. English code block with twelve discrete labelled blocks and inline `@tag` references.** Product tags get their own inline mentions — they anchor the PRODUCT SURFACE block. Seedance reference cap, version-conditional: roughly 9 references per prompt on 2.0, up to 50 on 2.5. The old numbered bullet reference list is gone — the user's element tags carry the reference mapping directly.
...
20. **Positive locks over negative prohibitions.** Same principle as cinema-director, whose § PHRASING: POSITIVE DEFAULT, SANCTIONED NEGATION BATTERIES heading is gone post-flip — the doctrine now lives in § WRITE THE VISIBLE as the bolded "Positive default, sanctioned negations." paragraph, and its sanctioned negation batteries are slot-indexed there (each battery named against the single slot it lives in) rather than listed as a standalone battery set.
...
Cinema-director now runs a **sixteen-slot spine** (drop 3, 19/08/2026). Each block below is reconciled against it individually — some survive intact, some were reshaped enough that the inheritance claim no longer holds, and one was deleted upstream entirely.

**Still identical in grammar, logic, and rules to cinema-director.** This skill does not redefine these. Use the cinema-director conventions for:

- **Frame Map** — 2D screen space, horizontal thirds, x/y% precision, depth layers, frame occupancy, negative space. Cinema-director § SLOT 6 — GEOMETRY MAP, which carries the same absolute-lateral-position, depth-plane and percentage-precision grammar. **This is a reversal:** the drop-2 fold recorded Frame Map as having no standalone equivalent upstream; drop 3 reinstated one, so it returns to shared grammar.
- **Element tags** — user-supplied semantic names (`@talent_ref`, `@perfume_ref`, `@bottle_cap_ref`, `@studio_plate`) replace `@imageN` numbering. Lowercase, underscore-separated, descriptive; character/talent refs use `_ref`, environment plates use `_plate`, product/prop tags use a descriptive noun. Ordering no longer matters — Seedance matches by tag name. Every tag named in the pre-prompt check must appear at least once inline in the code block. **The reference-count ceiling is version-conditional as of drop 3:** roughly 9 uploaded references per prompt on Seedance 2.0, up to 50 on Seedance 2.5. Establish the target version before planning the reference stack — cinema-director § STEP ZERO — TARGET VERSION CHECK. Full tag-naming rules and the canonical-over-plate rule: cinema-director § ELEMENT TAGS.

**Reshaped upstream — retained here as this skill's own legacy grammar, with the closest analogue named.** These are not renames. Write them to this skill's rules, not cinema-director's:

- **Subject Lock** — identity anchor per `@tag`, body orientation, pose, state, gaze, contact points, state-change details the reference cannot carry, lock-down line. Cinema-director merged Subject Lock, Prop Lock and Corps/Crew Lock into a single § SLOT 5 — ASSETS, written as one line per asset (`@tag = identity + THIS SCENE action + fidelity assertion`) rather than a standalone multi-line lock block. The identity-anchoring intent carries; the block shape does not.
- **Cross-Frame Rules** — no swap, no centre crossing, no depth change, distance consistency, screen sides held, eyelines, carry-across-the-cut. Cinema-director § SLOT 16 — LOCKS covers the same failure modes but inverts the form into a positive ordered chain of what must hold. This skill keeps the prohibition form.
- **World Plate** — location, time of day, weather, set dressing, colour palette, atmospheric quality, anchored to `@tag` if a plate is attached. Cinema-director folded this into the location asset line inside § SLOT 5 — ASSETS (`@x_plate = the location — …`, scoped to geography, materials, atmosphere and light direction only). No standalone block remains.
- **Movement** — four layers (character motion / micro-motion / environmental motion / camera motion) in flowing paragraph form with per-beat timestamps. Cinema-director's § MOVEMENT AND CHOREOGRAPHY heading no longer exists; subject motion and per-beat timing live in § SLOT 12 — ACTION TIMING and camera motion in § SLOT 9 — CAMERA.

**Deleted upstream with no successor — this skill's own block, full stop:**

- **Last Frame** — exact closing composition, on-screen text suppression line. Cinema-director retired the Last Frame block at drop 3 and did not replace it: the spine now opens on § SLOT 7 — FIRST FRAME, and the only end-state guidance left is a single rule inside § SLOT 12 — ACTION TIMING ("never end a shot on an action beginning"). There is no inheritance claim to make here anymore.

Any rule from cinema-director that still governs the identical blocks above applies here unchanged. This skill's additions are in the two new blocks (PRODUCT SURFACE and BRAND GRADE) and in the rewritten CAPTURE REALISM and SOUND BED sections below.
...
The Sound Bed describes only what the scene physically produces — same discipline as cinema-director, whose Sound Bed block became § SLOT 15 — AUDIO at drop 3.
...
**Audio modes:** this skill's own three modes (diegetic with ambient / silent capture / diegetic explicit no-music). Cinema-director's § SLOT 15 — AUDIO does not enumerate named modes, but the same diegetic-only default and lipsync sole-source hard lock apply. Note one drop-3 divergence not adopted here: cinema-director now carries a `NO BGM` convention promoted into its header block on silent scenes. This skill keeps its own no-music phrasing, because mode 2 already covers the case explicitly and commercial delivery routinely adds a licensed track in post. Mode 2 (silent capture) is common in commercial production when the music track is being added in post — flag this option if the user mentions a soundtrack.
...
The Capture Realism block in this skill serves the same structural purpose this skill has always used — separating the rendered-not-photographed failure modes — but is tuned for commercial production. Cinema-director merged its Capture Realism content into a single closing block at drop 2, then **dissolved that closer entirely at drop 3** — its contents now sit distributed across § SLOT 2 — STYLE PREFIX (texture, skin, technical invariants), § SLOT 8 — OPTICS, § SLOT 9 — CAMERA, § SLOT 10 — LIGHT & COLOUR and § SLOT 16 — LOCKS. There is no single capture-realism section left to point at. This skill keeps Capture Realism as its own dedicated block, separate from Camera Capture, with Product Surface and Brand Grade inserted between them.

**Philosophy shift:** cinema-director's target is a frame that reads as captured on a real camera operated by a real body — editorial grade, not commercial grade (its **House register.** paragraph in the skill preamble; there is no § CORE PHILOSOPHY heading post-flip, and the old "feels captured, not rendered" phrasing is gone with it). Here, the target is "looks controlled, precise, and real — not AI-generated." The anti-plastic discipline holds for skin. The depth-via-atmosphere mechanic holds for any scene with planes to separate. The contrast curve holds. What changes is the **moisture and specular logic**: product surfaces are not subject to the matte-only rule. That is handled in the PRODUCT SURFACE block. Capture Realism applies the matte rule to skin and non-product surfaces only.

**The four mechanics — commercial tuning:**

**1. Depth via suspended atmosphere between planes.** Same substance as cinema-director's own atmosphere discipline, which cinema-director writes as a full spine slot (§ SLOT 11 — ATMOSPHERE) rather than a Capture Realism sub-item; this skill keeps it folded inside Capture Realism instead of promoting it to a standalone block. One drop-3 divergence: cinema-director's atmosphere is now **source-bound** — vapour must have a named physical source — where this skill still specifies haze as a stated density percentage and visibility depth, which is what a controlled product frame needs. Always on when there are planes to separate. State the density in % and the visibility depth in meters — e.g., "haze 15%, readable to 50 meters" for a clean product-forward frame, scaling up for lifestyle or automotive exteriors with more atmosphere to separate. Any vehicle or moving-subject speed in the frame (C3 Automotive, C4 Lifestyle) is stated in km/h — never "fast," "slow." See cinema-director § WRITE THE VISIBLE.
...
**3. Per-zone specular kill on skin — with OPT-IN BEAUTY HIGHLIGHT exception.** Default: same core zones as cinema-director's Skin line, which now sits in its front-loaded § SLOT 2 — STYLE PREFIX — per-zone kill on forehead, nose bridge, cheekbones, temples, chin, and collarbones. Zero shine; real peach fuzz at jaw and hairline; real soft fine even pore texture; light absorbed like true subsurface scattering; warmth preserved. Flattering ceiling locked: no acne, no blemishes, no harsh pore detail.
...
**4. Contrast curve stated three ways.** This skill's own established mechanic — cinema-director's § SLOT 10 — LIGHT & COLOUR covers grade broadly, and adds a 70/20/10 colour-accent doctrine at drop 3, but it names no discrete "three ways" contrast-curve mechanic. Shadows lifted gently, highlights rolled off softly, nothing clipping or crushing; all specular surgically removed from skin (except declared opt-in zones) and non-product surfaces; grade low-contrast and slightly desaturated with warmth preserved.
...
FOV degree is the lens anchor across all five registers — same discipline as cinema-director. Write `[FOV°] ([mm])` in the prompt body, never mm alone; pick from cinema-director's discrete anchor ladder rather than an off-ladder degree. Full table and rationale: cinema-director § SLOT 8 — OPTICS (§ FOV degree anchor).
