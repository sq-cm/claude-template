...
**Adopted wholesale from upstream drop 3:** the STEP ZERO 2.0/2.5 target-version gate and its long-form anti-drift kit; SLOT 2 Style Prefix; the merged SLOT 5 Assets line (identity + THIS SCENE action + fidelity assertion) with state-conditional identity and inline anti-drift negation; SLOT 6 Geometry Map; SLOT 7 First Frame; SLOT 12 Action Timing; SLOT 13 Physics; SLOT 14 Acting; the 70/20/10 colour doctrine; source-bound vapour replacing the old blanket still-haze ban; the `NO BGM` convention; the eighteen-row repair table; and the optional negative-prompt block. Upstream drop 3 is largely Australian English already, with US residuals the flip handled unevenly on purpose — `vapor` swept to `vapour` at its five sites, `defense` retained at its two upstream-verbatim ones — as declared in the SWEEPS block below.
...
<!-- STUDIO-LOCAL SWEEPS: file-wide studio deltas marked collectively here, because an inline wrapper on every occurrence would be unreadable. (a) Frontmatter and H1 — `name:` is `cinema-director`, not upstream's `cinema-director-v3`, so the field matches the skill directory the loader validates against, and the H1 drops the `v3` with it; `description:` is a trimmed studio rewrite carrying the three-way routing disambiguation, per studio ruling #169 (trimmed skill descriptions are kept, upstream's long form is not folded in). YAML cannot carry HTML comments, so the divergence is declared here. (b) Locale — upstream drop 3 is predominantly Australian English but carries US residuals, and the flip corrected `vapor` to `vapour` at all five of its sites: the spine index row, the SLOT 11 — ATMOSPHERE opening line, the source rule, the PRE-DELIVERY PASS atmosphere row and the repair-table fog-machine row. The sweep is deliberately not complete — `defense battery` survives at its two upstream-verbatim sites (the unusual-FOV rule under SLOT 8 — OPTICS and its repair-table row), left as written rather than diverging further from upstream text, so this is a targeted correction and not a whole-file locale pass. (c) Reference grammar — upstream's numbered `@Image N` / `@Video N` handles are replaced by semantic element tags at four inline sites: the character asset example and the location-plate asset example under SLOT 5 — ASSETS, the opening-composition line under SLOT 7 — FIRST FRAME, and the attached-track sole-source lock under SLOT 15 — AUDIO. The grammar itself, and the retirement of the renumber-on-change rule with it, is declared and justified at ELEMENT TAGS (REPLACES NUMBERED IMAGE/VIDEO REFERENCES). -->
...
<!-- STUDIO-LOCAL BEGIN: timing-ownership graft — absent from upstream drop 3, which runs straight from the hand-timing step to the strobe-fights-lipsync paragraph. The studio adds it because the vocal take arrives as its own attached reference, and without an explicit ownership clause the model imposes an internal tempo that drifts against it. -->
```
...
<!-- STUDIO-LOCAL BEGIN: conversion note — rows below that name element tags, the tag list, the capture family or the mode label are converted from upstream's numbered-reference and single-capture-family forms, not additions to them. Two further rows are tightened rather than converted, and the tightening is the studio delta in each case. The First Frame row appends the completed-state clause so the check covers the STUDIO-LOCAL end-state graft in SLOT 12 — ACTION TIMING; upstream's row stops at the empty establishing hold. The Audio row appends the closing on NO BGM clause so the check requires the close that the NO BGM convention defines — the convention is adopted from upstream, but upstream's row asks for diegetic audio or the attached-track sole-source lock and does not require the NO BGM close. -->
<!-- STUDIO-LOCAL END -->
