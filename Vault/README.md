...
| `Chats/`     | Per-conversation chat workspaces — `[YYMMDD] [Chat Name]/`, created only via `/chat` |
...
There is deliberately no `Write(.claude/**)` / `Edit(.claude/**)` auto-approve grant in `.claude/settings.json` — and such a grant would be inert. Claude Code treats `.claude/` writes as protected-path writes and checks them *before* any allow rule from a settings file: they are never auto-approved — they prompt for confirmation or are routed to the auto-mode classifier, and in `dontAsk` mode they are denied (Claude Code permission-modes docs § Protected paths, checked 04/09/2026). The absence of the grant is therefore a statement of intent, not the enforcing mechanism — the enforcement is Claude Code's own. The intent stands: changes to the rules the team runs on should be a deliberate, surfaced act. Do not add the grant without a maintainer decision to do so.
